...
function clearSelectedSearchMatch() {
  document.querySelectorAll(".search-match-current").forEach((item) => {
    item.classList.remove("search-match-current");
...
function resetSearchState({ clearMessage = true, clearInput = false } = {}) {
  clearSearchMarks();
  activeSearchQuery = "";
  activeSearchMatches = [];
  activeSearchIndex = -1;
  activeSearchSignature = "";
  if (clearInput && siteSearchInput) siteSearchInput.value = "";
  if (clearMessage) updateSearchMessage();
}
...
function isSearchableTextNode(node) {
  ...
  return !parent.closest("header, script, style, noscript, form, button, input, select, textarea, .mega-menu, .site-search, .search-message, .search-match");
}

function getSearchRoots() {
  return [document.querySelector("main"), document.querySelector("footer")].filter(Boolean);
}
...
function wrapSearchMatches(textNode, ranges) {
  ...
  ranges.forEach(({ start, end }) => {
    if (start > cursor) {
      fragment.appendChild(document.createTextNode(textNode.nodeValue.slice(cursor, start)));
    }
...
function findPageMatches(query) {
  ...
  return textNodes.flatMap((node) => {
    const ranges = findTextRanges(node.nodeValue, searchTerm);
    return ranges.length ? wrapSearchMatches(node, ranges) : [];
  });
}

function resetSearchCycle(query) {
  clearSearchMarks();
  activeSearchQuery = query;
  activeSearchSignature = getSearchSignature(query);
  activeSearchMatches = findPageMatches(query);
  activeSearchIndex = -1;
}

function showActiveSearchMatch(match) {
  clearSelectedSearchMatch();
...
function updateSearchResultIndicator() {
  if (!activeSearchMatches.length) {
    updateSearchMessage("No matching content found.");
    return;
  }

  updateSearchMessage(`Result ${activeSearchIndex + 1} of ${activeSearchMatches.length}`);
}

// Handles Enter key submission and Search button clicks, cycling through each stored text match.
function runSiteSearch() {
  const query = getSearchTerm(siteSearchInput?.value || "");
  if (!query) {
    resetSearchState();
    return;
  }

  if (getSearchSignature(query) !== activeSearchSignature) {
    resetSearchCycle(query);
  }

  if (!activeSearchMatches.length) {
    updateSearchResultIndicator();
    return;
  }

  activeSearchIndex = (activeSearchIndex + 1) % activeSearchMatches.length;
  const match = activeSearchMatches[activeSearchIndex];
  showActiveSearchMatch(match);
  updateSearchResultIndicator();
  closeMobileMenu();
}

function clearSearchWhenClickingOutside(event) {
  if (!siteSearch || siteSearch.contains(event.target)) return;
  if (!siteSearchInput?.value && !activeSearchMatches.length) return;
...
if (siteSearch) {
  ...
  document.addEventListener("click", clearSearchWhenClickingOutside);
}
